...
impl TwoCentsApp {
    pub fn ui_expenses(&mut self, ui: &mut egui::Ui) {
      ...
      "Click or drag in a column to select rows. Hold Shift and drag to pan. Type to edit; Enter applies to all selected rows; Tab or Enter picks autocomplete and moves right; Escape clears.");
        ...
        let category_candidates = if editing {
            self.cached_category_candidates.clone()
        } else {
            Vec::new()
        };
        let member_candidates = if editing {
            self.cached_member_candidates.clone()
        } else {
            Vec::new()
        };
        let vendor_candidates = if editing {
            self.cached_vendor_candidates.clone()
        } else {
            Vec::new()
        };
        let description_candidates = if editing {
            self.cached_description_candidates.clone()
        } else {
            Vec::new()
        };
        ...
        let grid_res = crate::ui::components::grid_table_frame(ui).show(ui, |ui| {
        ...
        if let Some(col) = grid_res.clicked_sort_column {
            ...
            self.pending_expense_sort = Some(ExpenseSort {
                column: col,
                ascending,
            });
        ...
        self.deferred_category_commits
        ...
        if pending_total > 0 {
            ...
            let idle = self.expense_flush_at.map_or(true, |t| {
        ...
        for &(idx, field) in pending_updates {
            ...
            if field == "account" {
                ...
                if let Some(matched) = self
                    .accounts
                    .iter()
                    .find(|account| account.name.eq_ignore_ascii_case(row.account.trim()))
                {
