use eframe::egui::{self, color_picker::show_color_at, pos2, Color32, Id, RichText, Shape};
...
pub fn grid_header(ui: &mut egui::Ui, label: &str) -> egui::Response {
    ...
    let (rect, response) =
        ui.allocate_exact_size(egui::vec2(width, GRID_HEADER_HEIGHT), egui::Sense::click());
    ...
    ui.painter().text(
        egui::pos2(
            rect.left() + crate::ui::theme_tokens::SPACE_2,
            rect.center().y,
        ),
...
pub fn grid_text_field_committed(
    ui: &egui::Ui,
    response: &egui::Response,
    blur_block_rect: egui::Rect,
) -> bool {
...
pub fn grid_selection_anchor(selection: &Option<GridSelection>) -> Option<(GridColumn, usize)> {
    selection
        .as_ref()
        .and_then(|sel| sel.rows.first().copied().map(|row| (sel.column, row)))
}

pub fn grid_cell_editing(
    ...
    edit_cell
        .as_ref()
        .is_some_and(|&(col, idx)| col == column && idx == row)
}
...
pub fn paint_grid_cell_highlight(
    ...
    if dragging && selected {
        ui.painter().rect_filled(
            cell_rect,
            0.0,
            Color32::from_rgba_unmultiplied(sel.r(), sel.g(), sel.b(), 40),
        );
    } else if selected {
        ui.painter().rect_filled(
            cell_rect,
            0.0,
            Color32::from_rgba_unmultiplied(sel.r(), sel.g(), sel.b(), 15),
        );
...
pub fn process_grid_column_cell(
    ...
    if response.clicked() {
        let already_selected = selection.as_ref().map_or(false, |s| {
            s.column == column && s.rows.binary_search(&row).is_ok()
        });
        if !already_selected {
            *selection = Some(GridSelection {
                column,
...
pub fn category_picker_menu_width(ui: &egui::Ui, categories: &[Category], cell_width: f32) -> f32 {
    ...
    for category in categories {
        let pad = if category.parent_id.is_some() {
            36.0
        } else {
            24.0
        };
        ...
        candidates
            ...
            .filter(|candidate| {
                ...
                !candidate_lower.is_empty()
                    && (prefix.is_empty()
                        || (candidate_lower.contains(&prefix) && candidate_lower != prefix))
            })
            .cloned()
            .collect()
    };
...
pub fn autocomplete_suggestion_button(
    ui: &mut egui::Ui,
    label: &str,
    highlighted: bool,
) -> egui::Response {
...
pub fn member_none_row_selectable(ui: &mut egui::Ui, selected: bool) -> bool {
    ...
    let (rect, response) =
        ui.allocate_exact_size(egui::vec2(row_w, PICKER_ROW_HEIGHT), egui::Sense::click());
    if ui.is_rect_visible(rect) {
        if response.hovered() || selected {
            ui.painter()
                .rect_filled(rect, 2.0, crate::ui::theme::bg_hover());
        }
        ui.painter().text(
...
pub fn category_sub_picker_row(
    ui: &mut egui::Ui,
    label: &str,
    selected: bool,
    indent_px: f32,
...
pub fn category_picker_header_row(ui: &mut egui::Ui, label: &str, bg: Color32) {
    let row_w = ui.available_width().max(1.0);
    let (rect, _) =
        ui.allocate_exact_size(egui::vec2(row_w, PICKER_ROW_HEIGHT), egui::Sense::hover());
    if ui.is_rect_visible(rect) {
...
pub fn household_panel<R>(
    ...
    crate::ui::components::card_subtle(ui)
        .inner_margin(egui::Margin::symmetric(
            crate::ui::theme_tokens::SPACE_3 as i8,
            crate::ui::theme_tokens::SPACE_2 as i8,
        ))
        .show(ui, |ui| {
...
pub fn sorted_grid_indices<R: GridRow>(rows: &[R], sort: ExpenseSort) -> Vec<usize> {
    let mut indices: Vec<usize> = (0..rows.len()).collect();
    indices.sort_by(|&a, &b| {
        let cmp = match sort.column {
            ExpenseSortColumn::Date => rows[a].row_date().cmp(rows[b].row_date()),
            ExpenseSortColumn::Amount => {
                rows[a].row_amount_cents().cmp(&rows[b].row_amount_cents())
            }
            ExpenseSortColumn::Member => rows[a]
                .row_member()
                .to_lowercase()
                .cmp(&rows[b].row_member().to_lowercase()),
            ExpenseSortColumn::Category => rows[a]
                .row_category()
                .to_lowercase()
                .cmp(&rows[b].row_category().to_lowercase()),
            ExpenseSortColumn::Vendor => rows[a]
                .row_vendor()
                .to_lowercase()
                .cmp(&rows[b].row_vendor().to_lowercase()),
            ExpenseSortColumn::Description => rows[a]
                .row_description()
                .to_lowercase()
                .cmp(&rows[b].row_description().to_lowercase()),
            ExpenseSortColumn::Account => rows[a]
                .row_account()
                .to_lowercase()
                .cmp(&rows[b].row_account().to_lowercase()),
        };
        if sort.ascending {
            cmp
        } else {
...
pub fn ui_grid_date_edit(
    ...
    if editing {
        ...
        let mut parsed_date = parsed_date_result
            .clone()
...
pub fn accent_color_picker_ui(ui: &mut egui::Ui, color: &mut Color32) -> bool {
    ...
    ui.label(
        RichText::new("Choose an accent color")
            .small()
...
pub fn grid_nav_target(
    ...
    match nav {
        GridNav::NextRow => sorted_indices.get(row_pos + 1).map(|&idx| (column, idx)),
...
pub fn commit_grid_cell<R: GridRow>(
    ...
    let Some(source) = rows.get(idx) else {
        return Vec::new();
    };
    match column {
        GridColumn::Date => {
