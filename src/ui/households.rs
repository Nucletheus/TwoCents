...
impl TwoCentsApp {
    pub fn ui_households(&mut self, ui: &mut egui::Ui) {
            ...
            .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        if let Some(member) = &self_member {
                            if color_swatch_button(ui, member.color).clicked() {
                                member_color_pick =
                                    Some((member.id, member.name.clone(), member.color));
                            }
                        }
                        let self_field = grid_text_edit_cell(
                            ui,
                    ...
                    egui::ScrollArea::vertical()
                        ...
                        .show(ui, |ui| {
                            if other_members.is_empty() {
                                ui.label(
                                    RichText::new("No other members yet.")
                                        .color(crate::ui::theme::fg_secondary()),
                                );
                            }
                            for member in other_members {
                                ui.horizontal(|ui| {
                                    ...
                                    ui.with_layout(
                                        ...
                                        |ui| {
                                            if crate::ui::popups::styled_button(ui, "Remove", false)
                                                .clicked()
                                            {
                ...
                household_panel(ui, "Add member", |ui| {
                    let add_row = ui.horizontal(|ui| {
                        ...
                        let add_clicked = ui
                            .add(
                                egui::Button::new(RichText::new("Add").color(
                                    crate::ui::theme::contrast_text(crate::ui::theme::accent()),
                                ))
        ...
        if save_household {
            ...
            if !name.is_empty()
                && update_household_name(&self.conn, self.household_id, &name).is_ok()
            {
